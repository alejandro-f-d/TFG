...
\subsubsection{Incremento II: Gestión de identidades}
...
En este incremento se busca trabajar con APIs terceros para poder gestionar servicios asociados. Dentro de este paso estarían las conexiones con el GitLab, Microsoft Teams y los servicios de VPN.


\subsubsection{Incremento V: Auditoría y Cumplimiento}
En este incremento se busca la implementación de un módulo de trazabilidad para medir la robustez. 

Para ello, se tomarán como referencia las directrices del \textbf{NIST Cybersecurity Framework (CSF)}, específicamente en las funciones de \textit{Protect} (Protección) y \textit{Detect} (Detección). Se implementará un sistema de \textit{logging} exhaustivo que garantice la trazabilidad de todas las operaciones de provisión de servicios, cumpliendo con las recomendaciones del estándar \textbf{NIST SP 800-92} sobre gestión de registros de log.


\subsection{Estructura de los incrementos:}
El primer ciclo, es el único exento de esta estructura ya que se definen globalmente los requisitos que se deben cumplir en el sistema y las tecnologías que se van a usar en primera instancia.  

\subsubsection{Especificación y modelado:} 

En esta etapa se revisan los requisitos y las tecnologías que se van a usar en busca de detalles más específicos. En esta etapa se van a diseñar los diferentes casos de uso del incremento. 

\subsubsection{Diseño de la solución:}

Diseño de las pantallas que se van a utilizar y los botones que controlan las diferentes pantallas. Definición de la lógica que se va a utilizar en la implementación. 
...
En el apartado de validación y cierra se van a documentar casos de ataque asociados al incremento para verificar la seguridad de la aplicación o servicio.
