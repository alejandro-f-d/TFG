...
class ServiciosModel {
    static async getAllInfoServicios(page = 1, limit = 10, filtroNombre = "") {
        try {
            ...
            const res = await pool.query(SERVICIOS_QUERIES.GET_ALL_INFO, [limit, offset, busqueda]);
            return res.rows;
        } catch (error) {
    ...
    static async postServicio(uuidMaquina, datos) {
        ...
        const {
            nombreServicio, descripcionTecnica, entorno, publico, softwareBase,
            activo, nivelSeveridad, idUsuario, idPeticion, servidores, puertosAbiertos
        } = datos;

        try {
            await client.query("BEGIN");
            const resIdMaq = await client.query(SERVICIOS_QUERIES.GET_ID_MAQUINA, [uuidMaquina]);
            ...
            const resServicio = await client.query(SERVICIOS_QUERIES.POST_SERVICIO, [
                nombreServicio, descripcionTecnica, entorno, publico, softwareBase,
                activo, nivelSeveridad, idUsuario, idPeticion, uuidServicio
            ]);
            ...
            for (const puerto of puertosAbiertos) {
                await client.query(SERVICIOS_QUERIES.INSERT_PUERTO, [
                    puerto.numeroPuertoMaquina, puerto.protocolo, puerto.nombreServicio, 
                    puerto.puertoVirtual, idServicio
                ]);
            }
    ...
    static async getServicioByUuid(uuidMaquina, uuidServicio) {
        ...
        try {
            ...
            const resDevolver = await client.query(SERVICIOS_QUERIES.GET_BY_UUID_COMPLETO, [
    ...
    static async deleteServicioByUuid(uuidMaquina, uuidServicio) {
        ...
        try {
            await client.query("BEGIN");
            const resVerificar = await client.query(SERVICIOS_QUERIES.VERIFICAR_RELACION, [uuidServicio, uuidMaquina]);
            ...
            const resDelete = await client.query(SERVICIOS_QUERIES.DELETE_SERVICIO, [idServicio]);
    ...
    static async patchServicio(uuidMaquina, uuidServicio, camposCambiados) {
        ...
        try {
            await client.query("BEGIN");
            const resVer = await client.query(SERVICIOS_QUERIES.VERIFICAR_RELACION, [uuidServicio, uuidMaquina]);
            ...
            const camposPermitidos = ["nombreServicio", "descripcionTecnica", "entorno", "publico", "softwareBase", "activo", "nivelSeveridad"];
            const camposFiltrados = {};
            
            Object.keys(camposServicio).forEach(key => {
                if (camposPermitidos.includes(key)) camposFiltrados[key.toLowerCase()] = camposServicio[key];
            });
            ...
            if (servidores !== undefined) {
                ...
                if (Array.isArray(servidores)) {
                    for (const sId of servidores) await client.query(SERVICIOS_QUERIES.INSERT_CORRE, [idServicio, sId]);
                }
            }
            ...
            if (puertosAbiertos !== undefined) {
                ...
                if (Array.isArray(puertosAbiertos)) {
                    for (const p of puertosAbiertos) {
                        await client.query(SERVICIOS_QUERIES.INSERT_PUERTO, [p.numeroPuertoMaquina, p.protocolo, p.nombreServicio, p.puertoVirtual, idServicio]);
                    }
                }
            }
